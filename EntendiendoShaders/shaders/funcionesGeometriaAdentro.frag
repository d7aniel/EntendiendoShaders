...
float linea( vec2 p, float r )
{
    p*=4.;
    return  abs(sin(pow(p.x,2.)+pow(p.y,2.))-cos(p.x*p.y));
}
...
void main(){
    ...
    float fill = fillDistancia(distancia2,abs(sin(u_time*5.))*0.1);
